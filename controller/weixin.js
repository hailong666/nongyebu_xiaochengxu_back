...
module.exports = {
    ...
    try {
     ...
     await request(options,function(error,response,body){
        if (!error && response.statusCode == 200) {
          ...
          // // 解析用户数据
          // const wechatUserInfo = this.decryptUserInfoData(sessionData.session_key, fullUserInfo.encryptedData, fullUserInfo.iv);
   ...
   */
  signQuery(queryStr) {
    queryStr = queryStr + '&key=' + config.weixin.partner_key;
    const md5 = require('md5');
